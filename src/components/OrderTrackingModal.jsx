        ...
        <div className="bg-[#071b34] text-white p-5 flex items-center justify-between border-b border-amber-500/20">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-amber-200/80">HỆ THỐNG QUẢN LÝ TIẾN ĐỘ SẢN XUẤT HUNI</p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-2xl text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-amber-500"
              />
              <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
            </div>
            <button
              type="submit"
              className="px-6 py-3 bg-[#071b34] hover:bg-slate-800 text-amber-300 font-bold text-xs sm:text-sm rounded-2xl shadow transition-colors shrink-0"
            >
              Tra cứu
            </button>
          </form>
          ...
          {searched && searchedOrder ? (
            <div className="space-y-6">
              {/* Order Info Card */}
              <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200 text-xs text-slate-700 space-y-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <span className="text-slate-500">Mã đơn hàng: </span>
                    <strong className="text-amber-900 font-black text-sm">{searchedOrder.id}</strong>
                  </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-bold text-[#071b34] flex items-center justify-between">
                            <span>{st.title}</span>
                            {isCurrent && (
                              <span className="text-[10px] bg-amber-400 text-black px-2 py-0.5 rounded-full font-bold animate-pulse">
                                Đang thực hiện
                              </span>
                            )}
                          </div>
            ...
            <div className="text-center py-8 space-y-2">
              ...
              <a
                href={`tel:${BRAND_INFO.contact.hotlineRaw}`}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-500 text-[#071b34] font-bold text-xs rounded-xl mt-2"
              >
