      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-16 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 opacity-70">
            {partnerLogos.map((partner, idx) => (
              <div
                key={idx}
                className="px-4 py-2 bg-white rounded-xl border border-slate-200 shadow-2xs font-extrabold text-xs sm:text-sm text-slate-700 tracking-wider hover:text-amber-700 hover:border-amber-400 transition-colors"
              >
                {partner}
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((item, idx) => (
            <div
              key={idx}
              className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 hover:border-amber-400 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
            >
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-amber-700 font-semibold">{item.location}</span>
                </div>
              </div>
            </div>
          ))}
