...
export default function LogoCustomizerModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-cyan-400/40 animate-in fade-in zoom-in-95">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 max-h-[80vh] overflow-y-auto">
          ...
          <div className="md:col-span-6 bg-slate-900 p-6 flex flex-col items-center justify-center relative min-h-[380px]">
            <div className="relative w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl border border-slate-700 bg-slate-800">
              ...
              {position === "chest_left" && (
                <div className="absolute top-[32%] left-[34%] transform -translate-x-1/2 -translate-y-1/2 p-2 rounded border border-cyan-400/60 bg-black/40 backdrop-blur-xs flex items-center justify-center">
                  {logoType === "upload" && logoImage ? (
                    <img src={logoImage} alt="Uploaded logo" className="max-h-8 max-w-16 object-contain" />
                  ) : (
                    <span className="text-[11px] font-black tracking-wider text-cyan-300 drop-shadow-md">
                      {customText || "LOGO CTY"}
                    </span>
                  )}
                  <span className="absolute -top-4 text-[9px] bg-cyan-500 text-black px-1 rounded font-bold">
                    Ngực trái
                  </span>
                </div>
              )}
              ...
              {position === "back_center" && (
                <div className="absolute top-[40%] left-[50%] transform -translate-x-1/2 -translate-y-1/2 p-3 rounded border border-cyan-400/60 bg-black/40 backdrop-blur-xs flex items-center justify-center w-36">
                  {logoType === "upload" && logoImage ? (
                    <img src={logoImage} alt="Uploaded logo" className="max-h-12 max-w-full object-contain" />
                  ) : (
                    <span className="text-xs font-black tracking-wider text-cyan-300 text-center drop-shadow-md">
              ...
              {position === "sleeve_left" && (
                <div className="absolute top-[42%] left-[18%] transform -translate-x-1/2 -translate-y-1/2 p-1.5 rounded border border-cyan-400/60 bg-black/40 backdrop-blur-xs flex items-center justify-center">
                  {logoType === "upload" && logoImage ? (
          ...
          <div className="md:col-span-6 p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <div className="font-bold flex items-center justify-between">
                    ...
                    {method === "theu_vi_tinh" && <Check className="w-3.5 h-3.5 text-cyan-600" />}
                  </div>
                ...
                >
                  <div className="font-bold flex items-center justify-between">
                    ...
                    {method === "in_pet_4k" && <Check className="w-3.5 h-3.5 text-cyan-600" />}
                  </div>
            ...
            <div>
                ...
                <div className="flex items-center gap-2 text-xs">
                  <button
                    ...
                    className={`px-2 py-0.5 rounded font-semibold ${
                      logoType === "upload" ? "bg-cyan-100 text-cyan-800" : "text-slate-500"
                    }`}
                  >
                    Tải File Ảnh
                  </button>
                  <button
                    ...
                    className={`px-2 py-0.5 rounded font-semibold ${
                      logoType === "text" ? "bg-cyan-100 text-cyan-800" : "text-slate-500"
                    }`}
                  >
                    Gõ Tên Chữ
                  </button>
              ...
              {logoType === "upload" ? (
                <div className="border-2 border-dashed border-slate-300 hover:border-cyan-500 rounded-2xl p-4 text-center cursor-pointer bg-slate-50 relative">
                  <input
                    type="file"
                    accept="image/*"
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-xl text-xs text-slate-800 focus:outline-none focus:border-cyan-500"
                />
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2.5 border border-slate-300 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-cyan-500"
              />
            </div>
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-3.5 px-6 rounded-2xl bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600 text-[#071b34] font-black text-sm shadow-xl flex items-center justify-center gap-2 transform hover:-translate-y-0.5 transition-all"
              >
